...
import { Plus, Send, X, AlertCircle, RefreshCw, BarChart, CheckCircle2, Navigation, MousePointerClick, Trash2, Smartphone, BellRing } from "lucide-react";
...
export default function CampaignsPage() {
  ...
  const handleTestSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!testingCampaignId || !testToken) return;

    setTestSending(true);
    try {
      const res = await fetch(`/api/campaigns/${testingCampaignId}/send-test`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok) {
        alert(`Error: ${data.error || 'Failed to send test push'}`);
      } else {
        alert("Test push sent successfully to your device!");
      }
    } catch (err) {
              ...
              <div className="p-4 mt-auto bg-white flex items-center justify-end gap-2 text-sm z-10">
                {campaign.status === 'Draft' ? (
                  <>
                    ...
                    <button
                      onClick={() => {
                        setTestingCampaignId(campaign._id);
                        setIsTestModalOpen(true);
                      }}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-lg font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-300 active:scale-[0.98]"
                    >
                      <Smartphone className="w-4 h-4" /> Test
                    </button>
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-emerald-50/30">
              <h2 className="text-xl font-extrabold text-slate-800">Test Push Notification</h2>
              <button
                onClick={() => {
                  setIsTestModalOpen(false);
                  setTestToken("");
            ...
            <form onSubmit={handleTestSend} className="p-8">
              <div className="mb-6 bg-blue-50/50 text-blue-800 p-4 rounded-2xl flex items-start gap-3 border border-blue-100/50">
                <AlertCircle className="w-5 h-5 shrink-0 mt-0.5 text-blue-600" />
                <p className="text-sm font-medium leading-relaxed">Bypass topics and send instantly to specific devices using their exact FCM Tokens. You can paste multiple tokens separated by commas.</p>
              </div>

              <div className="mb-6">
                <label className="block text-sm font-bold text-slate-700 mb-2">Device FCM Tokens</label>
                <textarea
                  required
                  rows={4}
                  value={testToken}
                  ...
                  className="w-full font-mono text-sm px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#5f2299]/20 focus:border-[#5f2299] transition-all resize-y"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={testSending || !testToken}
                  className="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-md disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
